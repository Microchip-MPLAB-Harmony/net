...
uintptr_t DRV_ENCX24J600_SPI_SfrWrite(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance, DRV_ENCX24J600_SFR_MAP  reg, DRV_ENCX24J600_RegUnion  value,  uint8_t  opIndex )
{
    ...
    if (bankSelectResult == 4)
    {
        ...
        if (pBusInfo->bufferHandles[opIndex] == DRV_SPI_BUFFER_HANDLE_INVALID)
        {
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &rstCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
     }
     return ret;
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &erxCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &drxCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &rptCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &dpcCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &eiCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret =  DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &diCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &dfCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 ...
 uintptr_t DRV_ENCX24J600_SPI_FlowCtrlSingle(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance )
  {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &sfCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &mfCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &cfCmd, 1, NULL, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 uintptr_t DRV_ENCX24J600_SPI_WriteData(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance, DRV_ENCX24J600_POINTER  reg, uint8_t *  buffer, uint16_t  dataSize)
 {
     ...
     uintptr_t ret = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &(buffer[-1]), dataSize+1, DRV_ENCX24J600_SPI_OperationComplete, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
...
*/
 ...
 uintptr_t DRV_ENCX24J600_SPI_ReadData(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance, DRV_ENCX24J600_POINTER  reg, uint8_t *  buffer, uint16_t  dataSize)
 {
     ...
     uintptr_t ret = DRV_SPI_BufferAddWriteRead(pBusInfo->clientHandle, &rddCmd, 1, &(buffer[-1]), dataSize+1, DRV_ENCX24J600_SPI_OperationComplete, pDrvInstance);
     //Least intrusive fix for MH-4456
     if (ret == DRV_SPI_BUFFER_HANDLE_INVALID)
     {
         return 0;
