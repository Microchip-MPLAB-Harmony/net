...
static DRV_ENC28J60_BUS_RESULT _DRV_ENC28J60_OpStatusToResult(DRV_ENC28J60_OP_DCPT* pOpDcpt)
{
    DRV_SPI_BUFFER_EVENT spiEv = DRV_SPI_BufferStatus (pOpDcpt->spiHandle);
    
    if(spiEv == DRV_SPI_BUFFER_EVENT_COMPLETE)
    {   // success
        pOpDcpt->opStatus = DRV_ENC28J60_OP_STAT_SUCCESS;
        return DRV_ENC28J60_BR_SUCCESS;
    }
    else if(spiEv == DRV_SPI_BUFFER_EVENT_ERROR)
    {
...
static void DRV_ENC28J60_SPI_OperationStarting (DRV_SPI_BUFFER_EVENT event, DRV_SPI_BUFFER_HANDLE bufferHandle, void * context )
{
...
static void DRV_ENC28J60_SPI_OperationEnded (DRV_SPI_BUFFER_EVENT event, DRV_SPI_BUFFER_HANDLE bufferHandle, void * context )
{
...
static void _DRV_OpSetSpiHandle(DRV_ENC28J60_spiBusData* pBusData, DRV_ENC28J60_OP_DCPT* pOpDcpt, DRV_SPI_BUFFER_HANDLE spiHandle, bool autoAck)
{
...
static void DRV_ENC28J60_SPI_Acknowledge (DRV_SPI_BUFFER_EVENT event, DRV_SPI_BUFFER_HANDLE bufferHandle, void * context )
{
}
...
int32_t DRV_ENC28J60_SPI_OpenInterface(DRV_ENC28J60_DriverInfo *  pDrvInstance )
{
    ...
    DRV_SPI_CLIENT_DATA spiClientInfo =
    {
        ...
        .operationStarting = DRV_ENC28J60_SPI_OperationStarting,
        .operationEnded = DRV_ENC28J60_SPI_OperationEnded,
    };
...
{
    DRV_SPI_BUFFER_HANDLE   spiHandle;
    uint8_t*                pWrBuff;
    DRV_ENC28J60_OP_DCPT*   pOpDcpt;
    DRV_ENC28J60_spiBusData * pBusInfo = (DRV_ENC28J60_spiBusData *)pDrvInstance->busData;
    ...
    {   // Put in the data
        ...
        {   
            ...
            *(pWrBuff + 1) = regValue.value;
            ...
            if(spiHandle != DRV_SPI_BUFFER_HANDLE_INVALID)
            {
...
{
    DRV_SPI_BUFFER_HANDLE   spiHandle1, spiHandle2;
