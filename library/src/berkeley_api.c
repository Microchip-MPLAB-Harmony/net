...
void _cfgBsdSocket(struct BSDSocket * socketInfo)
{
    ...
    else if (socketInfo->SocketType == SOCK_STREAM)
    {
        ...
        {
            NET_PRES_SocketSignalHandlerRegister(socketInfo->SocketID, TCPIP_TCP_SIGNAL_RX_FIN | TCPIP_TCP_SIGNAL_RX_RST, TCP_SignalFunction, socketInfo);
        }
    }
...
static void TCP_SignalFunction(NET_PRES_SKT_HANDLE_T hTCP, NET_PRES_SIGNAL_HANDLE hNet, uint16_t sigType, const void* param)
{
    if((sigType & (TCPIP_TCP_SIGNAL_RX_FIN | TCPIP_TCP_SIGNAL_RX_RST)) != 0)
    {   // socket was closed
