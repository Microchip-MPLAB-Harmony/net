...
import { initBasicConfigLayer } from "./createGroupComponet";
...
const log = new Logger("BasicConfigLayer");
type Props ={
  componentIds:string[]
}
const BasicConfigLayer = ({componentIds}:Props) => {
  ...
  const [selectedComponentDeactivate,setSelectedComponentDeactivate]=useState<string[]>([]);
  ...
  const activate = async () => {
    ...
    await activateComponent("BASIC CONFIGURATION", selectedComponent).then(
      () => {}
    );
    setVisible(false);
...
export default BasicConfigLayer;
